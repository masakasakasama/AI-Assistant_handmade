...
| Luna vs Jev 初段ルーター比較 | v0.4.6で文字・音声の比較PoCを追加。比較中は物理操作を実行しない |
| 性能比較CLI | 日英独21ケース・4経路。初期データセットはスモーク用 |
| Galaxy STT / Android TTS / 音声キャンセル | v0.4.0で縦切り実装。v0.4.2で言語別STT切替・診断表示を追加。v0.4.3でBackend URLを初期設定。v0.4.4で日英独の自動言語検出を既定化。v0.4.5で音声状態と同期するマスコット表示を追加。v0.4.6でLuna / Jevの初段判定比較を追加。実Galaxy評価中 |
